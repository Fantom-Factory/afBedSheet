...
internal class AfxMultipartInStream : InStream {
	...
	** part. 
	static Void parseMultipart(InStream in, Str boundary, |Str:Str headers, InStream in| cb) {
		boundary = "--" + boundary
		line	:= in.readLine
		if (line == null)				return	// line will be null when bots don't respect the boundary 
		if (line == boundary + "--")	return
		if (line != boundary)			throw IOErr("Multipart form bad boundry, expecting ${boundary} got ${line}")
		
		try {
			while (true) {
				headers := WebUtil.parseHeaders(in)		// throws IOErr - Invalid CRLF line ending
			...
			// IOErrs are usually caused by bad bots - so not much to do other than log and return
			return
	}
	...
	** Note that this part *may* contain more than 'avail()' bytes.
	...
	private Bool gotMoreBytes() {
		try {
			...
			// sys::IOErr: Unexpected end of stream
			// this happens LOADS in the field when bots spam us with dodgy requests
			return false
	}
}
